...
func closeAllocatedChannel(c *Context) {
	if c == nil || c.allocated == nil {
		return
	}
	defer func() {
		_ = recover()
...
readLoop:
	for {
		...
		if err != nil {
			msg := strings.TrimSpace(string(accumulated.Bytes()))
			if msg == "" {
				msg = "no startup output; check browser path, profile lock, or desktop/session permissions"
			}
			return "", fmt.Errorf("chrome failed to start: %s", msg)
		}
		if forward != nil {
